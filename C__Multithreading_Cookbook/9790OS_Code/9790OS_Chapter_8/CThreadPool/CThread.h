...
public:
    ...
    ~CThread();
    void Start(LPVOID lpContext = 0) { pThreadContext->lpParameter = lpContext; ResumeThread(hThread); }
    ...
    DWORD ContextThreadId() const { return pThreadContext->dwThreadId; }
    DWORD ExitStatus() { GetExitCodeThread(hThread, &dwExitCode); return dwExitCode; }
    CThread* SetMaxWaitTime(DWORD dwMilliseconds) { pThreadContext->dwMaxWaitTime = dwMilliseconds; return this; }
...
DWORD WINAPI CThread::StartAddress(LPVOID lpParam)
{
    ...
    thread->pThreadContext->hThread = CreateThread(NULL,
                                                   0,
                                                   thread->pThreadContext->lpThreadStart,
                                                   thread->pThreadContext->lpParameter,
                                                   0,
