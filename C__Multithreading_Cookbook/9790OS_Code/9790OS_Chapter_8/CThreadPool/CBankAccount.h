...
{
public:
    CBankAccount(double dBalance) : dBalance(dBalance) { uID = NewId(); *szLock = 0;  _tcscpy_s(szLock, LockName()); }
    CBankAccount(double dBalance, TCHAR* szLockName) : dBalance(dBalance) { uID = NewId(); *szLock = 0; _tcscpy_s(szLock, szLockName); }
...
private:
    ...
    double dBalance;
    TCHAR szLock[32];
    static unsigned NewId() { static unsigned uSeed = 61524; return uSeed++; }
};
