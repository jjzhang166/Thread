...
​	尽管`std::future`可以处理所有必要的同步操作，用于线程间数据的传输，但是调用某一特殊` std::future`对象的成员函数，就会让这个线程的数据和其他线程的数据不同步。当多线程在没有同步的情况下，访问一个独立的`std::future`对象时，就会产生数据竞争和未定义的行为。这是因为：`std::future`模型独享异步结果的所有权，并且通过调用get()函数，一次性的获取数据，这就让并发访问变的毫无意义——只有一个线程可以获取结果值，因为在第一次调用get()后，就没有值可以再获取了。
...
​	采用`std::shared_future`时，独立对象上的成员函数是不同步的，所以为了在多个线程访问一个独立对象时，避免数据竞争，必须使用锁对访问进行保护。优先使用的办法：为了替代只有一个拷贝对象的情况，可以让每个线程都拥有自己对应的拷贝对象。这样，当每个线程都通过自己拥有的`std::shared_future`对象获取结果，那么多个线程访问共享同步结果就是安全的。可见图4.1。
