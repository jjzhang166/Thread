...
首先，检查查看参数是否是不同的实例，因为操作试图获取`std::mutex`对象上的锁，所以当其已经被获取时，结果很难预料(*undefined behavior*)。(一个互斥量可以在同一线程上多次上锁，标准库中`std::recursive_mutex`提供这样的功能。详情见3.3.3节)。然后，调用`std::lock()`①锁住两个互斥量，然后，分别为每一个互斥量创建`std:lock_guard`实例②③。`std::adopt_lock`参数表示`std::lock_guard`对象已经上锁，而且它们应该只能采用`mutex`中已存在的锁，而不是尝试在构造函数中锁住`mutex`。

这样就能保证在大多数情况下，函数退出时互斥量能被正确的解锁(保护操作可能会抛出一个异常)，也允许使用一个简单的“return”作为返回。还有，需要注意的是，当使用`std::lock`去锁lhs.m或rhs.m时，可能会抛出异常；这种情况下，异常会传播到`std::lock`之外。当`std::lock`成功的获取一个互斥量上的锁，并且当其尝试从另一个互斥量上再获取锁时，就会有异常抛出，第一个锁也会随着异常的产生而自动释放，所以`std::lock`要么将两个锁都锁住，要么一个都不锁。

当你同时获取两个以上的锁时，`std::lock`可以避免死锁，但是，它没办法帮助你获取其中任何一个锁。这时，作为开发人员，你不得不依赖于自己的经验来确保程序不会死锁。这并不简单：死锁是多线程编程中一个令人相当头痛的问题，并且死锁经常是不可预见的，因为在大多数时间里，所有工作都能很好的完成。不过，也一些相对简单的规则能帮助写出“无死锁”(*deadlock-free*)的代码。

### 3.2.5 避免死锁的进阶指导

虽然锁是产生死锁的一般原因，但也不排除死锁出现在其他地方。无锁的情况下，当每个`std::thread`对象调用join()相互等待时，两个线程之间就会产生死锁。这种情况下，没有线程可以继续运行，因为他们正在互相等待。这种情况很常见，一个线程会等待另一个线程，其他线程同时也会等待第一个线程结束，这不仅局限于两个线程之间，三个或多个线程的互相等待也会发生死锁。为了避免死锁，这里的指导意见为：当机会来临时，不要拱手让人。以下提供一些个人的指导建议，如何识别死锁，并消除其他线程的等待。

**避免嵌套锁**

第一个建议往往是最简单的：*一个线程已获得一个锁时，别再去获取第二个*。由于每个线程只有一把锁，因此，单从锁的使用角度来讲，很难会产生死锁。你也可能会在其他方面受到死锁的困扰(比如：线程间的互相等待)，但是，互斥锁是造成死锁的最常见的原因。如果你需要获取多把锁，可以采用`std::lock`来做这件事(对获取锁的操作上锁)，从而避免产生死锁。

**避免在持有锁时调用用户提供的代码**

第二个建议是次简单的：因为代码是用户提供的，你没有办法确定用户要做什么；用户程序可能做任何事情，包括获取锁。倘若你在持有锁的情况下，调用用户提供的代码，而用户代码也要获取一把锁，此时，这就违反了避免嵌套锁的原则，并且会产生死锁(有时，这是无法避免的)。当你正在写一份通用代码，例如3.2.3中的栈，对参数类型的每一个操作都是用户的代码定义的，在这种情况下，我们就需要其他的指导意见。

**使用固定顺序获取锁**

如果一定要获取两个以上(包括两个)的锁，那么，你不能使用`std::lock来单独获取它们，最好的方法是*在每个线程上，用固定的顺序获取它们*。3.2.4节中提到了一种获取两个互斥量时避免死锁的方法：关键是如何在线程之间，以一定的顺序获取锁。一些情况下，这种方式相对简单。比如，3.2.3节中的栈——每个栈实例中都内置有互斥量，但是对数据成员存储的操作上，栈就需要调用用户提供的代码。虽然，可以添加一些约束，对栈上存储的数据项不做任何操作，对数据项的处理仅限于栈自身。这会给栈增加一些负担，但是一个容器很少去访问另一个容器中存储的数据，即使发生了也会很明显，因此，这对于通用栈来说并不是一个特别沉重的负担。

其他情况下，就不会那么简单了，例如：3.2.4节中的交换操作，这种情况下你可能同时锁住多个互斥量(但是有时不会发生)。当回看3.1节中那个链表连接例子时，将会看到列表中的每个节点都会有一个互斥量保护。为了访问列表，线程必须获取他们感兴趣节点上的互斥锁。当一个线程删除一个节点，它必须获取三个节点上的互斥锁：要删除的节点，两个邻接节点(因为他们也会被修改)。同样的，为了让列表畅通(连接起来)，线程必须保证在获取当前节点的互斥锁前提下，获得下一个节点的锁，要保证指向下一个节点的指针不会同时被修改。一旦下一个节点上的锁被获取，那么第一个节点的锁就可以释放了，因为没有必要再持有它了。

这种“手递手”锁的模式允许多个线程访问列表，为每一个访问的线程提供不同的节点。但是，为了避免死锁，节点必须以同样的顺序上锁：如果两个线程试图用互为相反的顺序，使用“手递手”锁遍历列表，那么，执行到列表中间部分时将会发生死锁。当节点A和B在列表中相邻，当前线程可能会同时尝试获取A和B上的锁。另一个线程可能已经获取了节点B上的锁，并且试图获取节点A上的锁——经典的死锁场景。
...
这里提供一种避免死锁的方式，定义遍历的顺序。在不允许反向遍历的列表上，一个线程必须先锁住A才能获取B的锁，在锁住B之后才能获取C的锁，这将消除死锁发生的可能性。类似的约定常被用来建立其他的数据结构。

**使用锁的层次结构**

锁的层次结构是一种定义锁顺序的特殊情况，锁的层次的意义在于提供一种检查运行时约定是否被坚持的方法。这个建议需要对你的应用进行分层，并且识别在给定层上所有可能上锁的互斥量。当代码试图对一个互斥量上锁，在该层锁已被低层持有时，上锁是不允许的。你可以在运行时对其进行检查，通过分配层数到每个互斥量上，以及记录被每个线程上锁的互斥量。下面的代码列表中将展示两个线程如何使用分层互斥。
...
thread_a()⑥遵守规则，所以它运行的没问题。另一方面，thread_b()⑨无视规则，因此在运行的时候会失败。thread_a()调用high_level_func()函数，在函数内对high_level_mutex④上锁(其层级值为10000①)，然后为了获取high_level_stuff()函数所需要参数，采用该互斥量调用low_level_func()函数⑤，而low_level_func()函数会锁住low_level_mutex互斥量，这就没有问题了，因为这个互斥量有一个低层值5000②。

thread_b()运行就不会顺利了。首先，它锁住了other_mutex⑩，这个互斥量的层级值只有100⑦。这就意味着，它应该保护超低层级的数据(*ultra-low-level data*)。当other_stuff()函数调用high_level_func()⑧时，就违反了层级结构：high_level_func()试图获取high_level_mutex，这个互斥量的层级值是10000，要比当前层级值100大很多。因此hierarchical_mutex将通过抛出异常或者直接终止程序来报告错误。在层级互斥量上产生死锁，是不可能的，因为互斥量本身会严格遵循约定顺序，进行上锁。这也意味，*当多个互斥量在在同一级上时，不能同时持有多个锁，所以“手递手”锁的方案需要每个互斥量在一条链上，并且每个互斥量都比其前一个有更低的层级值，这在某些情况下无法实现。*

上面的例子也展示了std::lock_guard<>模板与用户定义的互斥量类型一起使用的情况。虽然hierarchical_mutex不是C++标准的一部分，但是它写起来很容易；一个简单的实现在列表3.8中展示出来。尽管它是一个用户定义类型，但是，它可以用于std::lock_guard<>模板中，因为它实现了互斥量所需的三个成员函数：lock()，unlock() 和 try_lock()。虽然你还没见过try_lock()怎么使用，但是其使用起来很简单：如果另外一个线程持有该互斥量上的锁，那么，它将返回false，而不是等待调用线程获取互斥量上的锁。在std::lock()函数内部，try_lock()会作为避免死锁算法的一部分。
...
​       这里重点是使用了thread_local的值来代表当前线程的层级值：`this_thread_hierarchy_value`①。它被初始化为最大值⑧，所以最初所有线程都能被锁住。因为其声明中有`thread_local`，所以每个线程都有其拷贝副本，这样线程中变量状态完全独立，当从另一个线程进行读取时，变量的状态也完全独立。参见附录A，A.8节，有更多与thread_local相关的内容。

​	第一次线程锁住一个`hierarchical_mutex`时，`this_thread_hierarchy_value`的值是`ULONG_MAX`。这个值大于其他任何值，因此，可以通过`check_for_hierarchy_vilation()`函数的检查②。采用这种检查方式，`lock()`函数可代表用于实际锁操作的内部互斥量④。一旦成功锁住，你就可以更新层级值了⑤。

​	当你持有第一个锁的同时，又锁住另一个`hierarchical_mutex`时，`this_thread_hierarchy_value`的值将会显示第一个互斥量的层级值。第二个互斥量的层级值必须小于已经持有的互斥量的层级值，这样才可以保证检查函数②通过。

​	现在，你可以通过调用unlock()函数来保存当前线程之前的层级值⑥；否则，即使线程没有持有任何锁，你也无法对一个具有较高层级值的互斥量进行上锁。由于仅仅当你持有`internal_mutex`③时才保存前一个层级值，且在解锁internal_mutex之前恢复它的层级值⑥，因此，你可以安全地在`hierarchical_mutex`内对其进行存储，这是因为`hierarchical_mutex`被内部互斥量的锁所保护着。

​	try_lock()与lock()的功能相似，除了在调用`internal_mutex`的`try_lock()`时会失败⑦，便也不能持有对应的锁，因此，不能更新层级值，并直接返回false。

​	虽然是运行时检测，但是它没有时间依赖性——不必去等待那些导致死锁出现的条件竞争。同时，设计过程需要去划分应用，在这样的情况下，互斥量可以消除产生死锁的可能性。这样的设计很有必要去做一下，即使你之后没有去做，代码也会在运行时进行检查。

**超越锁的延伸扩展**

​	如本节开头提到的那样，死锁不仅仅会发生在锁之间；死锁也会发生在任何同步构造中(可能会产生一个等待循环)，因此这方面也需要有指导意见，例如：和避免获取嵌套锁一样，等待一个持有锁的线程是一个很糟糕的决定，因为线程为了能继续运行可能需要获取对应的锁。类似的，如果去等待一个线程结束，我们有必要确定这个线程的层级，以便于线程只需要等待层级较低的线程即可。一个简单的办法就是：确定你的线程在相同的启动函数中已经joined，如同在3.1.2节和3.3节中描述的那样。

​	为了在代码中避免死锁，`std::lock()`和`std::lack_guard`覆盖了简单锁的大部分情况，但是有时需要更多的灵活性。对于这些情况，可以使用标准库提供的`std::unique_lock`模板。如` std::lock_guard`，这是一个参数化的互斥量模板类，并且它提供很多RAII类型锁用来管理`std::lock_guard`类型，可以让代码更加灵活。

### 3.2.6 std::unique_lock——灵活的锁

​	`std::unqiue_lock`使用更为自由的不变量，这样`std::unique_lock`实例不会总与互斥量的数据类型相关，使用起来要比`std:lock_guard`更加灵活。首先，可将`std::adopt_lock`作为第二个参数传入构造函数，对互斥量进行管理；***也可以将***`std::defer_lock`***作为第二个参数传递进去，表明互斥量在构造时应保持解锁状态***。这样，就可以通过`std::unique_lock`对象(不是互斥量)调用lock()函数或者传递`std::unique_lock`对象到`std::lock()`中来获取锁。采用`std::unique_lock`和`std::defer_lock`①，而非`std::lock_guard`和`std::adopt_lock`，我们可以很容易将清单3.6转换为清单3.9。代码长度相同，几乎等价，唯一不同的就是：***`std::unique_lock`会占用比较多的空间，并且比`std::lock_guard`稍慢一些***。保证灵活性要付出代价，这个代价就是：需要存储和更新信息来允许`std::unique_lock`实例不带互斥量。
...
列表3.9中，由于`std::unique_lock`支持lock()，try_lock()和unlock()成员函数，因此，能将`std::unique_lock`对象传递到`std::lock()`②。这些同名的成员函数在低层做着实际的工作，并且仅更新`std::unique_lock`实例中的标志来确定该实例是否拥有特定的互斥量，为了确保`unlock()`在析构函数中被正确调用，该标志是必须的。***如果实例拥有互斥量，那么析构函数必须调用unlock()；但当实例中没有互斥量时，析构函数一定不能调用unlock()***。可以通过`owns_lock()`成员变量来对这个标志进行查询。

​	可能如你期望的那样，这个标志被存储在某个地方。因此，`std::unique_lock`对象的体积通常要比`std::lock_guard`对象大，当使用`std::unique_lock`替代`std::lock_guard`时，因为会对标志进行适当的更新或检查，就会有一些轻微的性能劣势。当`std::lock_guard`已经能够满足你的需求，出于性能考虑，建议你继续使用它。当需要更加灵活的锁时，最好选择`std::unique_lock`，因为它更适合于你的任务。你已经看到一个延迟锁的例子，另外一种情况是锁的所有权需要从一个域转到另一个域。

### 3.2.7 不同域中互斥量所有权的传递

​	由于`std::unique_lock`实例没有与自身相关的互斥量，一个互斥量的所有权可以通过移动操作在不同的实例中进行传递。某些情况下，这种转移是自动发生的，例如：当函数返回一个实例时。而在其他情况下，需要显式的调用`std::move()`来执行移动操作。从本质上来说，需要依赖于源值是否是左值 —— 一个实际的值或是引用——或一个右值——一个临时类型。***当源值是一个右值时，所有权的转移自动发生；对于左值来讲，为了避免转移所有权过程出错，就必须显式进行移动***。`std::unique_lock`是一种可移动的但不可赋值的类型。附录A中A.1.1节有更多与移动语句相关的信息。

​	一种可能的用法是：允许一个函数去锁住一个互斥量，并且将所有权移到调用者上，因此，调用者可以在这个锁的保护下执行额外的动作。

下面的程序片段展示了：函数get_lock()锁住了互斥量，然后准备数据，并将锁返回给调用者：
...
​	lk在函数中被声明为自动变量，它不需要调用`std::move()`便可直接返回①(编译器负责调用移动构造函数)。process_data()函数能直接将所有权转移到`std::unique_lock`实例中②，do_something()函数可以使用预先准备的正确数据(数据没有受到其他线程的修改)。

​	***当要上锁的互斥量依赖于当前程序的状态或传入返回类型为`std::unique_lock`的函数的参数时，通常才会采用这样的用法***。一种使用方式是：不需要直接返回锁，而是作为网关类的一个数据成员，用于确保已经对保护数据的访问权限进行上锁。这种情况下，所有对数据的访问都必须通过网关类：当你想要访问数据，需要获取网关类的实例(如同前面的例子，通过调用get_lock()函数)来获取锁。之后你就可以通过网关类的成员函数对数据进行访问。当完成访问，可以销毁这个网关类对象，将锁进行释放，让别的线程来访问保护数据。这样的一个网关类可能是可移动的(以便于可以从一个函数返回)，在这种情况下锁对象数据数据也必须是可移动的。

​	`std::unique_lock`的灵活性同样也允许实例在销毁之前放弃其拥有的锁。可以使用unlock()来做这件事，如同一个互斥量：`std::unique_lock`的成员函数提供类似于锁定和解锁互斥量的功能。在`std::unique_lock`实例销毁前释放锁，意味着当没有必要再持有锁时，可以在特定的代码分支对其进行选择性的释放。这对于应用性能来说很重要，持有锁的时间增加会导致性能下降，因为其他线程会等待这个锁的释放，避免超越操作。

### 3.2.8 锁的粒度

​	3.2.3节中，已经对锁的粒度有所了解：锁的粒度是一个“摆手术语”(*hand-waving term*)，用来描述通过一个锁来保护的数据量大小。一个细粒度锁(*a fine-grained lock*)能够保护较小的数据量，一个粗粒度锁(*a coarse-grained lock*)能够保护较多的数据量。选择粒度对于锁来说很重要，为了保护对应的数据，保证锁有能力保护这些数据也很重要。我们都知道，在超市等待结账的时候，正在结账的顾客突然意识到他忘了拿蔓越莓酱，然后离开柜台去拿，并让其他的人都等待他回来；或者当收银员，准备收钱时，顾客才去翻钱包拿钱，这样的情况都会让等待的顾客很无奈。当每个人都检查了自己要拿的东西，且能随时为拿到的商品进行支付，那么的每件事都会进行的很顺利。
...
​	不需要让锁住的互斥量越过对process()函数的调用，可以在函数调用前对互斥量手动解锁①，并且在之后对其再次上锁②。

​	如果只有一个互斥量保护整个数据结构，不仅可能会有更多对锁的竞争，而且也会增加持有锁的时间。较多的操作步骤需要获取同一个互斥量上的锁，因此，持有锁的时间会更长。成本上的双重打击也算是为向细粒度锁转移提供了双重激励和可能。

​	如同上面的例子，合适的粒度不仅与需要上锁的数据量的大小相关，而且与锁的持有时间，以及持有锁时什执行什么操作相关。***一般情况下，执行必要的操作时，尽可能将持有锁的时间缩减到最小***。这也就意味着当持有锁时，不应该做一些浪费时间的操作，比如：获取另外一个锁(即使你知道这不会造成死锁)，或等待输入/输出操作完成，除非绝对需要。

​	清单3.6和3.9中，交换操作需要锁住两个互斥量，其明确要求并发访问两个对象。假设用来做比较的是一个简单的数据类型(比如:int类型)，将会有什么不同么？int的拷贝很廉价，所以可以很容易进行数据复制，并且每个被比较的对象都持有该对象的锁，在比较之后进行数据拷贝。这就意味着，在最短时间内持有每个互斥量，并且你不会在持有一个锁的同时再去获取另一个。下面的清单中展示了一个在这样情景中的Y类，并且展示了一个相等比较运算符的等价实现。
...
​	假设你有一个构建代码很昂贵的共享资源，它可能会打开一个数据库连接或分配很多的内存。延迟初始化(*Lazy initialization*)在单线程代码很常见 —— 每一个操作都需要资源进行初始化检查，如果没有初始化的话，需要在其使用前进行初始化：
...
​	如果共享资源对于并发访问是安全的，那么，将其转化为多线程代码时，仅仅需要保护的部分是初始化。①但是下面天真的转换会导致使用资源的线程产生不必要的序列化。这是因为每个线程必须等待互斥量来确定资源是否已经初始化。

清单 3.11 使用互斥量进行延迟初始化(线程安全)
...
​	这段代码相当常见，且存在不必要的序列化问题，很多人想出一些更好的办法来做这件事，包括声名狼藉的双重检查锁(*Double-Checked Locking*)模式：指针第一次读取数据不需要获取锁①，并且只有在指针为NULL时才需要获取锁。然后，一旦获取锁，就会再去检测指针② (这就是双重检查的部分)，避免其他线程在第一次检查和当前线程之间再进行初始化。
...
​	这个模式为什么声名狼藉呢？因为这里有潜在的条件竞争，因为外部的读取锁①没有与内部的写入锁进行同步③。因此就会产生条件竞争，这不仅覆盖指针本身，还会影响到指针指向的对象；虽然一个线程知道另一个线程已经完成对指针的写操作，但是，它可能没有看到新创建的some_resource实例，这就导致do_something()④操作一个不正确的值。这个例子是一种典型的条件竞争，C++标准将其定义为“数据竞争”，并认为它是“未定义行为”。这种竞争肯定是可以避免的。可以阅读第5章，那里有更多对内存模型的讨论，包括数据竞争的构成。

​	C++标准委员会也认为条件竞争的处理很重要，因此，***C++标准库提供了`std::once_flag`和`std::call_once`来处理这种情况。比起锁住互斥量，并显式的检查指针，每个线程只需要使用`std::call_once`即可，在`std::call_once`的调用返回时，就能安全的知道指针已经被其他的线程初始化了***。使用`std::call_once`比显式使用互斥量消耗的资源更少，特别是当初始化完成后。下面的例子展示了与清单3.11中的同样的操作，这里使用了`std::call_once`。在这种情况下，初始化通过调用函数完成，同样这样操作使用类中的函数操作符来实现同样很简单。如同标准库中的大多数函数一样，或作为函数被调用，或作为参数被传递，`std::call_once`也可以和任何函数或可调用对象一起使用。
...
​	在这个例子中，`std::once_flag`①和正在初始化的数据都是命名空间区域的对象，而`std::call_once()只能用于类成员的延迟初始化，如同下面的例子一样：
...
​	在这个例子中，第一次调用send_data()①或receive_data()③的线程完成初始化工作。使用成员函数`open_connection()`去初始化数据，也需要将this指针传进去。和标准库中的其他函数一样，可以接受可调用对象，比如`std::thread`的构造函数和`std::bind()`。通过向`std::call_once()`②传递一个额外的参数来完成这个操作。

​	值得注意的是，`std::mutex`和`std::one_flag`的实例不能拷贝和移动，所以当你使用它们作为类成员函数，如果你需要用到他们，你就得显示定义这些特殊的成员函数。

​	**还有一种情形在初始化过程中存在潜在的条件竞争**：其中一个局部变量被声明为static类型。这种变量在声明后就已经完成初始化；对于多线程调用的函数，这就意味着这里有条件竞争——抢着去定义这个变量。对于C++11之前的很多编译器(译者：不支持C++11标准的编译器)，在实践过程中，这样的条件竞争是确实存在的，因为在多线程中，每个线程都认为他们是第一个初始化这个变量线程，或者或者在初始化开启另外一个线程之后，同时在它完成之前，其他线程尝试使用该变量。**在C++11标准中，这些问题都被解决了**：初始化及定义完全在一个线程中发生，并且没有其他线程可以在初始化完成前对其进行处理，条件竞争终止于初始化阶段，这样比在之后再去处理好的多。在只需要一个全局实例的情况下，这可以作为`std::call_once`的替代方案
...
​	只在初始化时保护数据，是许多通用场景中的一种特殊情况：对于很少有更新的数据结构来说，在大多数情况下，这种数据结构是只读的，并且对于多线程并发读取操作也是很愉快的，不过一旦数据结构需要更新，那么就会有条件竞争的产生。必须承认，这的确也需要一种保护机制。

### 3.3.2 保护很少更新的数据结构

​	试想，为了将域名解析为相应的IP地址，将DNS缓存到一张表中。通常情况下，一个给定的DNS将会在很长的一段时间内保持不变——在很多情况下，DNS入口很多年都保持不变。虽然，随着时间的推移，在用户访问不同网站时，新的入口可能会添加到表中，但是这些数据很有可能在其生命周期内保持不变。定期检查缓存中入口的有效性，就变的十分重要了，即使仅仅是细节改变了，这也需要进行更新。

​	虽然更新频度很低，但更新也是有可能发生的，当缓存被多个线程访问时，在更新过程中，需要对其进行适当的保护，用来确保线程读到缓存中的有效数据。

​	在没有使用专用数据结构的情况下，这种方式是符合预期，并且为并发更新和读取特别设计的(更多的例子在第6和第7章中介绍)。这样的更新要求更新线程独占数据结构的访问权，直到完成更新操作。一旦改变完成，数据结构对于多线程并发访问又会是安全的。使用一个`std::mutex`来保护数据结构有些反应过度，因为在没有发生修改时，它将削减并发读取数据的可能性；这里需要另一种不同的互斥量，这种新的互斥量常被称为**“读者-写者锁”（*reader-writer mutex*），因为其允许两中不同的使用方式：一个“写”线程独占访问和共享访问，多个“读者”线程并发访问**。

​	新的C++标准库应该不提供这样的互斥量，虽然已经提交给了标准委员会[3]。因为建议没有被采纳，这个例子在本节中使用的实现是Boost库提供的(Boost采纳了这个建议)。你将在第8章看到，这种锁的也不能包治百病，其性能依赖于参与其中的处理器数量，同时也与读者和更新者线程的负载有关。为了确保增加复杂度后还能得到受益，在目标系统上测试代码性能就很重要了。

​	比起使用`std::mutex`实例进行同步，不如使用`boost::shared_mutex`来做同步。**对于更新操作，**可以使用`std::lock_guard<boost::shared_mutex>`和`std::unique_lock<boost::shared_mutex>`作为`std::mutex`的替代方案来进行上锁。与`std::mutex`一样，这就能保证更新线程的独占访问；**对于不需要修改数据结构的线程**，可以使用`boost::shared_lock<boost::shared_mutex>`来获取共享访问权。这与使用`std::unique_lock`一样，除非多线程同时获取同一个`boost::shared_mutex`上的共享锁。**唯一的限制就是**：当任一线程拥有一个共享锁时，尝试获取独占锁的线程将会阻塞，直到其他线程放弃他们的锁；同样地，当任一线程拥有一个独占锁时，其他线程就无法获得共享锁或独占锁，直到第一个线程放弃其拥有的锁。
...
​	当一个线程已经获取一个`std::mutex`实例时(已经上锁)，并对其再次上锁，这个操作就是错误的，并且继续尝试这样做的话，会产生未定义行为。然而，在某些情况下，当一个线程尝试获取同一个互斥量多次，而没有对其进行一次释放，这样的操作是可行的。为了达到这样的目的，C++标准库提供了`std::recursive_mutex`类。其功能与`std::mutex`类似，除了你可以从同一线程的单个实例上获取多个锁。在其他线程锁住互斥量前，你必须释放你拥有的所有锁，因此，当你调用lock()三次时，你也必须调用unlock()三次。正确使用`std::lock_guard<std::recursive_mutex>`和`std::unique_lock<std::recursice_mutex>`可以帮你处理这些问题。

​	大多数情况下，当你认为你需要一个嵌套锁时，可能要对你的设计进行改动。**嵌套锁一般用在可被多线程并发访问的类上，所以它拥有一个互斥量来保护成员数据**。每个公共成员函数都会对互斥量上锁，然后完成对应的功能之后再解锁互斥量。不过，有时一个公共成员函数会调用另一个公共函数作为其操作的一部分。在这种情况下，第二个成员函数也会试图锁住互斥量，这就会导致未定义行为的发生。“变通的”(*quick-and-dirty*)解决方案会将互斥量转为嵌套锁。这样第二个成员函数就能成功的进行上锁，并且函数能继续执行。

​	但是，这样的使用方式是不推荐的，因为其过于草率，并且不合理。特别是，当锁被持有时，类的不变量通常是正在被修改的，这就意味着，当不变量正在改变的时候，第二个成员函数还需要继续执行。**一个比较好的方式是，从中提取出一个函数作为类的私有成员，并且让所有成员函数都对其进行调用**，这个私有成员函数不会对互斥量进行上锁(在调用前必须获得锁)。然后，你仔细考虑一下，当调用新函数时，数据的状态。
